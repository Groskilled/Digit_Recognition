...
class Network(object):
    ...
    def update(self, batch, lrate):
        ...
        for x, y in batch:
            ...
            nabal_b = [nb + dnb for nb, dnb in zip(nabla_b, dnabla_b)]
            nabal_w = [nw + dnw for nw, dnw in zip(nabla_w, dnabla_w)]
    ...
    def gradient_descent(self, data, size, cicles, lrate):
        m = len(data)
        for i in xrange(cicles):
            random.shuffle(data)
            batches = [data[j: j+size] for j in xrange(0,m,size)]
            for batch in batches:
                self.update(batch, lrate)

    def evaluate(self, X):
        test_results = [(np.argmax(self.feedforward(x)), y) for (x, y) in X]
        return sum(int(x == y) for (x, y) in X)
